...
(function(jQuery,_){
	...
	_.loading = function(txt){
		if(txt === false){
			...
			var div = jQuery('<div class="qp_lodediv" style="background: url('+yyuc_jspath+'/img/loadb.png);border-radius:10px;min-width: 269px;height: 90px;position:fixed;left: -1000px;top: -1000px;text-align: center;"><span style="color: #ffffff;line-height: 90px;font-size: 22px; white-space: nowrap;">&nbsp;&nbsp;&nbsp;<img src="'+yyuc_jspath+'/img/load.gif" style="vertical-align: middle;"/>&nbsp;&nbsp;'+txt+'</span></div>');
			jQuery('body').append(div);
			div.css('zIndex',29891014);
			div.css('left',parseInt((jQuery(window).width()-div.width())/2));
			var top = parseInt((jQuery(window).height()-div.height())/2);
			div.css('top',top);
		}	
	}
