...
function getFlashAdvs(){
	...
	$.getJSON(g_action + "get_top_pic.html",
			{
				"r" : Math.random()
			},
			function(json){
...
function showFlashAdvs(json){
	...
	for(var i = 0;i<len;i++){
		...
		if(item.picsrc.indexOf(".swf") != -1){
			imgBuffer.push("<li><embed src=\""+item.picsrc+"\" quality=\"high\" wmode=\"transparent\" pluginspage=\"http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash\" type=\"application/x-shockwave-flash\" width=\"910\" height=\"200\"></embed></li>");
		}else{
			var img_attr = "data-url=\""+item.picsrc+"\"";
			if(i == 0){
				img_attr = "src=\""+item.picsrc+"\"";
			}
			imgBuffer.push("<li><a href=\""+tempUrl+"\"><img "+img_attr+" width=\"910\" height=\"200\" alt=\"\" /></a></li>");
		}
...
function showAppNesseary(json){
	...
	obj_wrap.find(">a.prev").addClass("prev-disable");
	if(g_nesseary_pagecount > 1){
		obj_wrap.find(">a.next").removeClass("next-disable");
	}
}
//必备分页效果
function pageAppNesseary(pk){
	...
	if(pk == "prev"){
		...
		g_nesseary_pageno++;
		g_nesseary_animate = true;
		obj_wrap.find(">a.prev").removeClass("prev-disable");
		if(g_nesseary_pageno >= g_nesseary_pagecount-1){
			obj_wrap.find(">a.next").addClass("next-disable");
		}
	}
	...
	while(i<len){
		...
		if(!obj_icon.attr("src") || obj_icon.attr("src")=="" || obj_icon.attr("src").indexOf("/media/images/index/default-icon.jpg")!=-1){
...
function getDailyChosen(){
	...
	*/
	$.getJSON(g_action + "dayrecommend_web",
			{
				"pageNo" : 1,
				"pageSize" : 15,
				"r" : Math.random()
...
function getAppRankList(cid){
	...
	$.getJSON(g_action + "get_zhrank.html",
			{
				"ranktype" : 0,
				"cid" : cid,
				"cmstype" : cmstype,
...
function getHotCategory(){
	...
	*/
	$.getJSON(g_action + "qryhotcate_web",
			{
				"hotcatenum" : 6,
				"icontype" : 48,
				"pagesize" : 5,
...
function showHotCategory(json){
	...
	for(var i=0; i<len; i++){
		...
		if(etyle == "soft"){
			...
			shtml.push("					<img class=\"img_scrolloading_"+lmid+"\" src=\""+g_respath+"/media/images/index/default-icon.jpg\" data-url=\""+item.icon+"\" width=\"48\" height=\"48\" itemprop=\"photo\" alt=\""+UI.htmlAttributeEncode(item.softname)+"\" />");
			//shtml.push("					<div class=\"shadow\"></div>");
			shtml.push("				</a>");
			shtml.push("			</div>");
			shtml.push("			<p class=\"app-name\"><a href=\"43354545435435345?appid="+item.appid+"&icfa="+item.icfa+"&lmid="+lmid+gf_param+"\">"+UI.htmlEncode(item.softname)+"</a></p>");
